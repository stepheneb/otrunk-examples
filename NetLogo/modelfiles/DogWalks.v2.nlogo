      ...
      [ set xcor mouse-xcor
        ifelse car-number-dragging = 1                                                    ;;  then set the car's xcor to the mouse coordinate
          [set x-car1-world x-world car-x-pos 1
            ...
            if xcor - x-car1-mouse-previous < 0 and shape != "dog airedale left"                   ;;change direction of car so it is always going forward
              [set shape "dog airedale left"]                                                     ;;with car image for shape the heading DOES NOT WORK, for some mysterious reason
            if xcor - x-car1-mouse-previous > 0 and shape != "dog airedale right"                  ;;instead of changing heading we will change shape
          ...
          [set x-car2-world x-world car-x-pos 2
            if xcor - x-car2-mouse-previous < 0 and heading != "dog airedale left"
              [set shape "dog airedale left"]
            if xcor - x-car2-mouse-previous > 0 and heading != "dog airedale right"
              [set shape "dog airedale right" ]
...
to set-car-position [ car-num pos-world ]  ;note that pos-world MAY be outside the legal range of the "world"
  ...
  ask cars [                                                                               ;  there is a little "extra' space (should be 0.5) on each end
    if car-number = car-num [
      ...
      if new-mouse-pos - x-mouse-previous < 0 and shape != "dog airedale left"          ;;change direction of car so it is always going forward
              [set shape "dog airedale left"]                                           ;;with car image for shape the heading DOES NOT WORK, for some mysterious reason
      if new-mouse-pos - x-mouse-previous > 0 and shape != "dog airedale right"         ;;instead of changing heading we will change shape
              [set shape "dog airedale right" ]
