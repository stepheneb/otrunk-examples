...
class Questions
  ...
  ## Return text for user's answer to a survey question
  ## (CAPA Survey activity only)
  ...
  def isGradable(question)
    question.is_a?(OTQuestion) and 
    (question.input.is_a?(OTChoice) or 
     question.input.is_a?(OTEmbeddedTextInput) or
     question.input.is_a?(OTDataTable) or #limited: WSU multichoice digital circuit only
     question.input.is_a?(OTObjectSet))   #linited: WSU multichoice digital circuit only
  end
