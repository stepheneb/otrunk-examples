...
To setup-globals
  ...
  set tmax 60                      ; sets the time allowed for the game
...
to setup-space ; create a bunch of stars and the green zone
  ask patches [
    if pxcor < (min-pxcor + 10 ) [           ;set the safe zone
      set pcolor 51 ]]
  create-stars 200 [   
...
to check-end                                               ;check whether the rocket has successfully returned and stopped
  if count shuttles > 0 [
    if first [towed?] of shuttles [                        ;if the shuttle is being towed
      ask rockets [if pcolor = 51 and abs x-vel < .1 [     ;and if the towing rocket is at rest in the green area....
...
590
115
The shuttle on the right has run out of fuel\nUse the rocket to bring back the shuttle within 60 sec. \nTo score, you have to stop in the dark green zone with the shuttle\nThe rocket can fire only once every two seconds
14
0.0
1
