...
globals [t graph-rocket graph-shuttle shuttle-mass fuel-remaining rocket-x-vel shuttle-x-vel] 
...
to setup-rockets                            ; this version creates one rocket
  ...
  ask rockets [                             ; set the variables that are common to all rockets
    ...
    set graph-rocket x-pos - min-pxcor    ; initialize graphing variable
    ]
...
to setup-shuttles  ; these are the blue shuttles that are not onboard rockets at the start of the game.
  ...
  create-shuttles 1 ; 
    [ set sx-pos ( 0 )  set sy-pos 0 ]  ; bob's original value for sx-pos = max-pxcor - 5 //the center position is 0
  ask shuttles [
    set size 8
    set shape "ufo side"
    set color blue
    ...
    set graph-shuttle sx-pos - min-pxcor  ; initialize graph value
      ]
end
...
to go
  ...
  every .5 [
    ask shuttles [set graph-shuttle sx-pos - min-pxcor]
    ask rockets [set graph-rocket x-pos - min-pxcor]]
end

to move-rockets
  ask rockets [
    set x-pos (x-pos + .05 * rocket-x-vel)          ; advance the rocket using its current velocity
...
to pickup-shuttle ; picks up a shuttle if a rocket is near it and sufficiently near its velocity
  ask rockets [
    let vx rocket-x-vel let vy y-vel let m mass let w who
    ask shuttles in-radius 2 [
      if not towed? [
      let closing-speed sqrt ( (vx - shuttle-x-vel) ^ 2 + (vy - sy-vel) ^ 2 ) ;
...
true
0
Polygon -7500403 true true 5 150 250 40 205 150 250 260
