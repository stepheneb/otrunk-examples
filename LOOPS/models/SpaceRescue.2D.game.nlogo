...
globals [t tmax fuel-remaining blackout score stopped?] 
...
; stopped? determines whether the game is stopped
...
to setup
  ...
  setup-rockets
  setup-flames
  setup-space                     ; sprinkle some stars at random
...
To setup-globals
  ...
  set tmax 80                     ; sets the time allowed for the game
  set score 0                      ; initialize the score to zero
  set stopped? false               ; allow the game to run
end
...
to setup-space ; create a bunch of stars and the green zone
  ask patches [
    if pxcor < (min-pxcor + 16 ) and abs pycor < 8 [           ;set the safe zone
      set pcolor 51 ]]
  create-stars 200 [   
    set size .3 * (2 + random 6) 
    ...
    ifelse on-screen? x y [
      ...
      [die]                                  ; kill the star if it is off-screen
    ]
  ask patches [                              ; setup barrier
    if abs pxcor < 2 and abs pycor < (max-pycor - 8 ) [
      set pcolor red]]
end
...
to fire-up
    ...
    ask rockets with [color = red] 
      [if countdown <= 0 [
        set heading 0
        set y-vel y-vel + 100  / mass
        fire-rockets 0 x-pos y-pos 
        set countdown blackout]]]
end
...
to go
  every .05 [                                     ; move rockets and shuttles every 50 msec and advance time
    ...
    set t t + .05                                 ; advance time
    if stopped? or t >= tmax [stop]]              ;  ...and see whether time is up
  every .2 [                                      ; every 2-tenths of a second
    ...
    check-end  ]                                  ; check to see whether the game has been completed
  every .4 [                                      ; every four-tenths of a second manage the puffs
     ask rockets with [on-screen? x-pos y-pos]    ; for each rocket on-screen drop a dot marking the rocket's path
      [ hatch-puffs 1 [set color white set size 1 set shape "cloud" ]] ; create a puff at the on-screen rocket
    ask puffs [
      set color color - .4                        ; fade all puffs and kill them off when they become black
...
to check-explode                         ; if a rocket or shuttle encounters a red patch, explode and stop the game
  ask rockets [if pcolor = red [ explode set stopped? true ]]
  ask shuttles [if pcolor = red [ explode set stopped? true]]
end
    ...
    ask shuttles in-radius 2 [
      if not towed? [
        ...
        if closing-speed <= 5 [     ; the rocket can pick up the shuttle
          ...
          set vy (m * vy + shuttle-mass * sy-vel ) / (m + shuttle-mass)
                    set m m + shuttle-mass             ; increase the mass of the rocket 
          set color red
          set towed? true  
          set by-whom w]]    ]                              ; the shuttle is now being towed
    set mass m  set x-vel vx set y-vel vy                   ; this extracts rocket variables computed inside 'ask shuttles'                     
    ]
end

to check-end                                               ;check whether the rocket has successfully returned and stopped
  if count shuttles > 0 [
    if first [towed?] of shuttles [                        ;if the shuttle is being towed
      ask rockets [if pcolor = 51 and ( x-vel ^ 2 + y-vel ^ 2 ) < .1 [     ;and if the towing rocket is at rest in the green area....
        set score 500 + 100 * ( 4 * fuel-remaining  +  tmax - t ) ; the score is 200 + a bonus for fuel and time remaining
        set stopped? true ]]]]
end
...
590
115
Use the rocket to bring back the shuttle within 80 sec. \nTo score, you have to stop in the dark green zone with the shuttle\nYour score depends on how quickly you return and how much fuel is left\nWatch out for the red no-fly zones!!
14
0.0
1
...
WHAT IS IT?
-----------
This version of Rocket Rescue focuses attention on two-dimensional motion. This makes a far more difficult game. A barrier is added to force the user to go up and down. 

HOW IT WORKS
------------
This is an accurate simulation of frictionless motion. Impulses are delivered by the rocket motor that change velocity. The mass determines how much a given impulse chages the velocity. When the rocket and shuttle combine, their masses add and the effect on velocity of an impulse is less. 

HOW TO USE IT
-------------
The four buttons each deliver an impulse to the rocket or rocket-shuttle combination. This can be used as a two-person game, with one controlling left-right with keys A and S and a second person controlling up and down using keys O and L on the right of the keyboard. Sometimes the keys don't work--if this happens, focus the computer by clicking in a blank space near the keys 
...
THINGS TO TRY
-------------
Try to use the rocket to pick up the shuttle and bring it back to the left side to the dark green patch. If you stop on the shuttle, you will pick it up. 
