...
globals [t graph-rocket graph-shuttle shuttle-mass fuel-remaining] 
...
to setup-rockets                            ; this version creates one rocket
  ...
  ask rockets [                             ; set the variables that are common to all rockets
    ...
    set graph-rocket x-pos - min-pxcor
    ]
  set fuel-remaining 15
end
...
to setup-shuttles  ; these are the blue shuttles that are not onboard rockets at the start of the game.
  ...
  create-shuttles 1 
    [ set sx-pos (max-pxcor - 5)  set sy-pos 0 set sx-vel -15]
  ask shuttles [
    set size 8
    set shape "ufo side"
    set color blue
    ...
    set graph-shuttle sx-pos - min-pxcor
      ]
end
...
to update-graph
  set-current-plot-pen "Rocket Position"
  plot graph-rocket
  set-current-plot-pen "Shuttle Position"
  plot graph-shuttle
end
...
590
138
The shuttle on the right has run out of fuel\nFire the rocket left and right to bring back the shuttle within 30 sec. \nThis is a practice game. Harder games follow.\nTo start the game, press \"Setup\" and then \"Start/stop\"\nTry to conserve fuel but don't run into the walls!! 
14
0.0
1
...
414
336
What do you notice that is different about the return trip? \nCan you explan the difference? 
11
0.0
1
...
true
PENS
"Rocket Position" 0.5 0 -2674135 true
"Shuttle Position" 0.5 0 -13345367 true
